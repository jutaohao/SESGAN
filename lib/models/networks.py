...
class Dis(nn.Module):
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        main = nn.Sequential()
        main.add_module("Linear",nn.Linear(in_features = nz, out_features =30))
        main.add_module("Linear2",nn.Linear(in_features = 30, out_features =1))
        main.add_module('Sigmoid', nn.Sigmoid())
        self.main=main
    def forward(self, input):
        input = input.view(-1, 100).squeeze(1)
        output = self.main(input)
        return output
#
...
class NetG(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.decoder1 = Decoder(opt.isize, opt.nz, opt.nc, opt.ngf, opt.ngpu, opt.extralayers)
        self.discriminator = define_D(opt, norm='batch', use_sigmoid=False, init_type='normal')

    def forward(self, x):
        latent_i = self.encoder1(x)
        gen_imag1 = self.decoder1(latent_i)
        pred_real, feat_real = self.discriminator(x)
        pred_fake, feat_fake = self.discriminator(gen_imag1)

        return gen_imag1,latent_i,pred_real, feat_real,pred_fake, feat_fake
...
def define_D_SE(opt, norm='batch', use_sigmoid=False, init_type='normal'):
    netD = None
    norm_layer = get_norm_layer(norm_type=norm)
    netD = BasicDiscriminatorSE(opt)
    return init_net(netD, init_type, opt.gpu_ids)
...
# Defines the submodule with gannomaly connection.
# X -------------------identity---------------------- X
